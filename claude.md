...
- **Policy — scripted with injected failures (calibration):** `src/sim/scripted.py`. Knobs steer behavior toward intended visual failure modes: `action_noise>0` destabilizes the approach (tends to produce `missed_approach`), `angle_deg>0` offsets the target laterally (clean `missed_approach`), `premature_close=True` pins the gripper shut from step 0 (`gripper_not_open`), `grip_scale<0.7` carries the cube up for `SLIP_CARRY_STEPS` then releases (`gripper_slipped`; adapter demotes transient-success cases to failure). Ground truth comes from human labels on a sampled subset, not from the knob — see `src/human_labels.py`. Lift-only.
...
- **Outcome-axis taxonomy (3 labels).** Collapsed from a 10-label mechanism taxonomy to `missed_approach` / `gripper_slipped` / `gripper_not_open` (+ `other` escape). The fine-grained distinctions (scratch vs knock vs approach_miss; slip vs premature_release) were below the pixel+frame-rate resolution of the judge and were the main source of multiclass confusion. See `docs/taxonomy.md`.
- **Single-call vision judge.** One Messages-API call per failed rollout at 1920 px × `clamp(video_duration*3, 12, 36)` frames. Prompt rewritten around the 3-label outcome axis — no per-frame CoT table, no anti-default heuristics. Binary success still comes from `RolloutResult.success` (sim `env._check_success()`); the judge only classifies the failure mode and points at it (or abstains via `point=null` on no-contact failures).
...
- Live smoke to confirm the simplified judge lifts multiclass label accuracy on the 3-label axis (no prior validation yet under the new prompt).
...
- Do not skip the post-success re-verification in `src/sim/adapter.py`. It demotes transient success crossings (e.g. slip rollouts where the cube is briefly above the success height before falling) to failures. Without it, `gripper_slipped` rollouts register as sim-successes and never reach the judge.
...
- **Judge simplification history.** Two-pass (coarse 768 px → fine 2576 px windowed) was retired on 2026-04-24 in favor of a single-call design with per-frame CoT over 12–36 frames and a 10-label taxonomy. That version then misclassified most runs in live testing — the per-frame observation ritual created ~100 places to mis-observe before label selection, and one anti-default heuristic ("resist approach_miss") was flipping correct answers on BC-RNN's approach failures. **Current design:** one Messages-API call at **1920 px** (the max-dimension cap for many-image requests) over `clamp(video_duration*3, 12, 36)` frames, no per-frame table, no anti-default heuristics, three outcome-axis labels (`missed_approach` / `gripper_slipped` / `gripper_not_open` + `other`). Telemetry rows are still appended as a supporting ASCII table. Keyframe comes from the judge's `frame_index`, red dot from `point` (skipped when `null`).
