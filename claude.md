...
**The two populations are load-bearing.** *Calibration rollouts* run a scripted policy with deliberately-injected failures — ground truth is known by construction, so we measure judge precision/recall against it. *Deployment rollouts* run a real policy (today: a pretrained BC-RNN) where ground truth is unknown — the judge is applied with its calibration trust attached. The dashboard's headline number is the calibrated judge applied to the deployment population.
...
└──────────────────────────────────────────────────────────────────┘
                                ▲
┌──────────────────────────────────────────────────────────────────┐
│ Layer 2: Two populations of policies                             │
│  Calibration: scripted IK picker with injected failure knobs     │
│               (Lift today, Nut planned) → known ground truth     │
│  Deployment:  robomimic BC-RNN pretrained (NutAssemblySquare)    │
│               → no ground truth, judged with calibration trust   │
└──────────────────────────────────────────────────────────────────┘
                                ▲
┌──────────────────────────────────────────────────────────────────┐
│ Layer 1: robosuite + MuJoCo                                      │
│  Lift (calibration today)  ·  NutAssemblySquare (deployment)     │
│  Custom camera for Nut: midpoint(frontview, agentview) + 30 cm   │
└──────────────────────────────────────────────────────────────────┘
```
...
- **Simulator:** `mujoco` + `robosuite` ≥ 1.5. Two envs in active use: **`Lift`** (calibration env — scripted policy with injected failures gives known ground truth for judge measurement) and **`NutAssemblySquare`** (deployment env — pretrained BC-RNN ~18-100% failure rate depending on seed, no GT label). Custom camera (midpoint of frontview+agentview, dollied 30 cm back) applied automatically when `env_name == NutAssemblySquare` — see `src/sim/cameras.py`.
- **Policy — pretrained (deployment):** `robomimic` v0.3.0 with a pretrained BC-RNN checkpoint for NutAssemblySquare (`artifacts/checkpoints/square_ph_low_dim.pth`, fetched by `scripts/fetch_checkpoints.py`). Note: robomimic 0.3.0 was trained against robosuite 1.4.x; obs-spec drift to 1.5 is the known sharp edge — wrap any mismatch in `src/sim/pretrained.py`, never fork the policy. Do **not** use LeRobot — its policies target ALOHA / Koch / SO-100, not Franka Panda.
...
- **Both populations integrated.** Scripted Lift (calibration, ground truth via injection knobs) + pretrained BC-RNN NutAssemblySquare (deployment, no GT). Camera override for Nut: midpoint(frontview, agentview) + 30 cm pull-back. Post-success hold of 20 frames so clean rollouts show "cube clearly held aloft" — fixes a Pass-1 false-positive class.
- **Two-pass vision judge.** Pass-1 at 24 frames × 768 px (denser than initial 14, prompt asks for the *earliest* failure window not the consequence frame). Pass-2 at 14 frames × 2576 px windowed on the coarse range with 8-frame padding, prompt explicit about temporal reasoning between adjacent frames and named anti-bias against `default-to-approach_miss`.
...
- **Dashboard.** Live tab (chat + current rollout video + rich gallery), Judge calibration tab (cohort pills + binary panel with Wilson 95% CI + multiclass heatmap + per-label table + drill-down filter), Deployment findings tab with **Judge Trust banner** at the top + cluster cards decorated with calibration-precision chips. Phase progress strip below the banner shows X/N for each phase live. Population chips (amber / steel-blue) on every rollout card.
- **Plain-language agent messages.** System prompt includes a translation table — agent.message events translate "knob" → "failure-injection parameter", "scripted policy" → plain English, etc. Internal thinking can stay technical.

### Open methodology questions

- **Cross-task calibration drift.** Today the judge is calibrated on Lift (scripted, with injected failures) and applied to NutAssemblySquare (pretrained, no GT). The Lift calibration number is at best a *floor* on judge competence, not the deployment number. Dashboard labels this honestly via the Judge Trust banner copy and the per-label `judge P = X (Lift)` chips. **Closing the gap requires either:** (a) a scripted Nut policy with injectable failures (proposed work; mirrors the Lift pattern with three knobs: `approach_offset`, `grip_force_scale`, `placement_offset`), or (b) a small human-labeled Nut held-out set (~15 videos × 30 sec). Option (a) is methodologically stronger, ~3–5 h work.
...
- **Cross-task calibration drift.** Today the judge is calibrated on Lift (scripted, with injected failures) and applied to NutAssemblySquare (pretrained, no GT). The Lift number is at best a *floor* on judge competence on Nut, not the deployment number. The Judge Trust banner labels this honestly, and per-label deployment-finding chips render as `judge P = X (Lift)` rather than bare `X`. Closing the gap requires either a scripted Nut policy with injectable failures (proposed; ~3-5 h) or a small human-labeled Nut held-out set. Don't quote the calibration number as if it transferred — it doesn't.
