...
- **Human-reviewer baseline** (`baseline_cost_for`) — what the same eval would cost if a robotics engineer ran it manually. The clock starts the moment the run is launched, because "running an eval" includes staging the policy and watching the rollouts, not just classifying outcomes. Modeled as $75/hr × 3 min/rollout × N. This baseline accumulates regardless of whether any API call has fired — it represents the human alternative, not the pipeline. Constants `BASELINE_HOURLY_RATE_USD` / `BASELINE_SECONDS_PER_ROLLOUT`.
