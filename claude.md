...
- **Policy — pretrained:** `robomimic` v0.4.0 with a pretrained BC-RNN checkpoint for NutAssemblySquare. Sibling of robosuite, checkpoints on HuggingFace, installs painlessly. Do **not** use LeRobot — its policies target ALOHA / Koch / SO-100, not Franka Panda, and the obs/action shapes don't match.
